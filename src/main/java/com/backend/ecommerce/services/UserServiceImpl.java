...
public class UserServiceImpl implements UserService {
  ...
  public UserDto patchUserName(UUID id, String firstName, String lastName) {
    Optional<User> findUser = userJpaRepo.findById(id);
    if (findUser.isPresent()) {
      User patchedUser = findUser.get();
      patchedUser.setFirstName(firstName);
      patchedUser.setLastName(lastName);
      userJpaRepo.save(patchedUser);
      return userMapper.toUserDto(patchedUser);
    } else throw new NoSuchElementException(ErrorConstants.ErrorMessage.USER_DOES_NOT_EXIST);
  }


  public UserDto updateUserName(UUID id, UserNameEditDto userNameEditDto) {
    Optional<User> find = userJpaRepo.findById(id);
    if (find.isEmpty()) {
      throw new NoSuchElementException(ErrorConstants.ErrorMessage.USER_DOES_NOT_EXIST);
    }
    find.map(user -> {
      user.setFirstName(userNameEditDto.firstName());
      user.setLastName(userNameEditDto.lastName());
      return user;
    });
    User updatedUser = userJpaRepo.save(find.get());
    return userMapper.toUserDto(updatedUser);
